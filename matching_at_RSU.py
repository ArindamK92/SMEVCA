...
def match(ev_list: List[EV], cp_list: List[CP], Pref: Dict[int, List[Any]], func):
    ...
    while(True):
        ...
        if len(S_dash) > 0:
            for s_i in S_dash:
                # print(_Pref[s][0])
                choice_tuple = _Pref[s_i][0] # (c_j, d_ij, t0_ij, psi_ij, r_i, gamma_i) Get the best CP choice
                _Pref[s_i].remove(choice_tuple) # remove the CP from pref. list
                choice_CP_ID = choice_tuple[0]
                updated_tuple = (s_i,) + choice_tuple[1:] # (s_i, d_ij, t0_ij, psi_ij, r_i, gamma_i) # update with the EV ID
